...
- 结构化数据：MySQL 主从 + 分库分表预案（按 `user_id` 分片；当单表 > 5000 万行或单库 > 2TB 时启动分片，初始 8 分片并按 2 倍扩容）。
- 非结构化文本（长复盘、附件）：对象存储。
...
4. 标签映射：依据阈值输出强项/短板标签（0.6/0.4 的权重用于保证可解释规则稳定性优先，同时保留语义理解能力；权重可在配置中心按实验组动态调整）。  
...
- 升级公式：
  ...
  - 边界示例：`XP=0 -> L1`，`XP=100 -> L2`，`XP=300 -> L3`，`XP≈51100 -> L10`（超过 L10 后进入同级段位成长，不再升主等级）。
- 防刷机制：
  - 同题短时间重复作答收益衰减。
  - 仅“有效训练时长 >= 最小阈值”计入经验。
...
- 规范：`.env` 管理环境变量，禁止明文密钥入库；密钥统一托管于 Secrets Manager（如 AWS Secrets Manager / Vault），通过 CI/CD 在运行时注入到 K8s Secret，开发环境使用本地加密模板文件。
