...
- 结构化数据：MySQL 主从 + 分库分表预案（按 `user_id` 一致性哈希分片）。
  - 分片触发阈值：单表 > 5000万行或单库 > 2TB。
  - 阈值依据：在压测基线下，接近该规模时 P95 查询时延和维护窗口显著上升，需提前分片保障 SLA。
...
3. 维度评分归一化：
   ...
   - 默认权重依据：优先保证可解释性与评分稳定性（规则分），同时用语义分补充复杂表达理解；当评分一致性 < 85% 或 14 日提升率下降 > 10% 时触发权重实验调整。
...
- 探索利用平衡：
  - 80% 下发高收益题型
  - 20% 探索新题型/新难度
  - 比例依据：冷启动与常规阶段验证中 80/20 在完成率与提升率上综合最优；高等级用户（L>=7）或低活跃用户（近 7 日完成 < 3）可动态调整为 70/30。
...
- 经验值：
  ...
  - 连续训练奖励：阶梯加成
- 升级公式：
  - `level = min(10, floor(log2(total_xp / 100 + 1)) + 1)`
...
- 分支策略:
  - 分支类型: `main` (生产)、`develop` (集成)、feature 分支。
  - PR 要求: feature 必须通过 PR 合并, 至少 1 名 Reviewer 通过。
  - 质量门禁: CI (lint/test/security scan) 全绿后方可合并。
  - 合并策略: 默认使用 squash merge。
  - 保护策略: `main` 启用 branch protection (禁止直推)。
- 发布流转: feature -> `develop` 完成联调与回归, 按发布窗口由 `develop` 合并至 `main`; `main` 发布前需通过 staging 验收与关键指标门禁。
